    ...
    mFilename(apFilename)
{
    ...
    std::string pn=iUtils->gCacheDir()+"/"+iUtils->app_name()+"-"+apFilename+".db";
    
    iUtils->checkPath(iUtils->gCacheDir());
    
//    logErr2("pn %s",pn.c_str());

    const int ret = sqlite3_open_v2(pn.c_str(), &mpSQLite, aFlags, apVfs);
    if (SQLITE_OK != ret)
    {
    ...
    mFilename(aFilename)
{
    ...
    std::string pn=iUtils->gCacheDir()+"/"+iUtils->app_name()+"-"+aFilename+".db";
    
    iUtils->checkPath(iUtils->gCacheDir());
    
//    logErr2("pn %s",pn.c_str());
    const int ret = sqlite3_open_v2(pn.c_str(), &mpSQLite, aFlags, aVfs.empty() ? nullptr : aVfs.c_str());
    if (SQLITE_OK != ret)
    {
        const SQLite::Exception exception(mpSQLite, ret); // must create before closing
        sqlite3_close(mpSQLite); // close is required even in case of error on opening
