...
public:
    ...
    inline int exec(const QUERY& aQueries)
    {
        return exec(aQueries.prepare().c_str());
    }
     ...
     * @throw SQLite::Exception in case of error
     */
    inline Column execAndGet(const QUERY& aQuery)
    {
        return execAndGet(aQuery.prepare().c_str());
    }
