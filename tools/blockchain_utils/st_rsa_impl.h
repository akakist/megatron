...
class st_rsa_impl
{
    std::string encrypt(const std::string &m, int (*func)(int, const unsigned char*, unsigned char*, RSA*,int)) const;
...
public:
    ...
    std::string privateEncrypt(const std::string &m) const;
...
public:
    ...
    static std::string rand_bytes(int n);
